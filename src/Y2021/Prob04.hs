...
rows = [1..5]
bingos = S.fromList <$> [(x `Vec2`) <$> rows | x <- rows] <> [(`Vec2` y) <$> rows | y <- rows]
...
score calls board = (sum . IS.toList $ IM.keysSet board IS.\\ IS.fromList called) * last called
  where called = take (fstBingo calls board) calls

readBoard :: [String] -> IM.IntMap (Vec2 Int)
readBoard ls = IM.fromList [(read n, Vec2 x y) | (x, l) <- zip rows ls, (y, n) <- zip rows $ words l]
